...
WHERE A.NUMDE=B.NUMDE AND A.NUMEM<>B.DIREC AND FECNA IN (SELECT MAX(FECNA) FROM TEMPLE GROUP BY NUMDE)
GROUP BY A.NUMDE, NOMEM, NUMHI
HAVING AVG(COMIS)=(SELECT MAX(AVG(COMIS)) FROM TEMPLE GROUP BY NUMDE) 
```
SELECT NOMEM, NUMHI
FROM TEMPLE A, TDEPTO B
WHERE A.NUMDE=B.NUMDE AND A.NUMEM<>B.DIREC AND FECNA IN (SELECT MAX(FECNA) FROM TEMPLE A, TDEPTO B WHERE A.NUMDE=B.NUMDE AND A.NUMEM<>B.DIREC GROUP BY A.NUMDE)
GROUP BY A.NUMDE, NOMEM, NUMHI
HAVING AVG(COMIS)=(SELECT MAX(AVG(COMIS)) FROM TEMPLE GROUP BY NUMDE) 
...
OBTENER POR ORDEN ALFABÉTICO LOS EMPLEADOS CON UNN APELLIDO DE 7 LETRAS
```sql
SELECT * FROM TEMPLE
WHERE NOMEM LIKE '"_______,%%'
ORDER BY NOMEM
...
HALLAR EL DEPARTAMENTO EN EL QUE TRABAJA EL EMPLEADO CON MAYOR ANTIGUEDAD SIENDO SU SALARIO EL SALARIO MÁS BAJO
```sql
SELECT DISTINCT NUMDE
FROM TEMPLE
